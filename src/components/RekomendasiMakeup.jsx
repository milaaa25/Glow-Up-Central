...
function RekomendasiMakeup() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const makeup = localData.default.filter(p => p.category === 'makeup').slice(0, 6)
        setProducts(makeup)
      } catch (err) {
  ...
  return (
    <section id="rekomendasi-makeup" style={{ width: '100%', background: '#fdf6f0', borderTop: '1px solid #e8c5b5' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '6rem 4rem' }}>
        ...
        <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(2.2rem, 4vw, 3.2rem)', fontWeight: 300, color: '#6b3e2e', marginBottom: '3rem' }}>
          Rekomendasi <em style={{ fontStyle: 'italic', color: '#d4876b' }}>Makeup</em>
        </h2>
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '4rem', color: '#9b7e70' }}>
            ...
            <p style={{ fontSize: '0.85rem' }}>Memuat produk makeup...</p>
        ...
        {!loading && !error && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            {products.map(product => (
              <div
                key={product.id}
                style={{ background: 'white', border: '1px solid #e8c5b5', borderRadius: 4, overflow: 'hidden', transition: 'transform 0.25s, box-shadow 0.25s', cursor: 'pointer' }}
...
export default RekomendasiMakeup